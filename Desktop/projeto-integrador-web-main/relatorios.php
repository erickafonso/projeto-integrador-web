...
function buscarContas($conn, $categoria, $forma_pagamento, $data_inicio, $data_fim, $valor_min, $valor_max)
{
    ...
    if (!empty($valor_min) && !empty($valor_max)) {
        $sql .= " AND c.valor BETWEEN ? AND ?";  // Filtro entre valor mínimo e máximo
    }
    ...
    // Se existir intervalo de datas
    if (!empty($data_inicio) && !empty($data_fim)) {
        $paramTypes .= 'ss'; // Tipo string para ambas as datas (data_inicio e data_fim)
        $params[] = $data_inicio;
        $params[] = $data_fim;
    }
    // Se existir filtro de valor mínimo e máximo
    if (!empty($valor_min) && !empty($valor_max)) {
        $paramTypes .= 'dd';  // Tipo decimal para ambos os valores (mínimo e máximo)
        $params[] = $valor_min;
        $params[] = $valor_max;
    }
            ...
            <div class="form-group">
                ...
                <select id="categoria" name="categoria">
                    ...
                    <?php foreach ($categorias as $cat) : ?>
                        <option value="<?php echo $cat['nome']; ?>" <?php echo ($filtro_categoria == $cat['nome']) ? 'selected' : ''; ?>><?php echo $cat['nome']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="forma-pagamento" name="forma_pagamento">
                    ...
                    <?php foreach ($formas_pagamento as $fp) : ?>
                        <option value="<?php echo $fp['nome']; ?>" <?php echo ($filtro_forma_pagamento == $fp['nome']) ? 'selected' : ''; ?>><?php echo $fp['nome']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
