...
$despesasPeriodo = buscarDespesasPorIntervalo($pdo, $usuarioId, $dataInicio, $dataFim);
...
foreach ($despesasPeriodo as $despesa) {
    $nomesDespesas[] = $despesa['nome'];
    $valoresDespesas[] = $despesa['valor'];
    $totalGeral += $despesa['valor'];
}

function buscarDespesasPorIntervalo($pdo, $usuarioId, $dataInicio, $dataFim) {
    $sql = "SELECT nome, valor FROM despesa 
...
<head>
    ...
    <title>Gastos por Despesas</title>
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="grafico-container">
            <h2>Distribuição de Gastos por Despesas</h2>
    ...
    <script>
        ...
        const despesasLabels = <?= json_encode($nomesDespesas) ?>;
        const despesasValores = <?= json_encode($valoresDespesas) ?>;
            ...
            chart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Valor (R$)',
                        data: despesasValores,
                        backgroundColor: tipoGraficoAtual === 'bar' ? cores : cores.slice(0, despesasLabels.length),
                        borderWidth: 1
                    }]
                },
                options: {
